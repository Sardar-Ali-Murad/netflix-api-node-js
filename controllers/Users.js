...
import { ConstructionOutlined, PasswordTwoTone, Unarchive } from "@mui/icons-material"
...
const getAllUsers=async (req,res)=>{
    let currentUser=await Auth.find({_id:req.user.userId})
    if(currentUser.role!=="admin"){
        throw new UnAuthenticatedError("You cannot do this action")
    }
...
const updateUser=async (req,res)=>{
    ...
    if(!name || !email || !password){
        throw new BadRequestError("Please Provide the name email and the password to proceed")
    }
